...
% Terms of use: 
% Academic: Cite (Thickness difference paper), 
...
%
% The section fits performs Drude fits on the complex conductivity data.
%
% Other m-files required: a lot
% MAT-files required: refractiveIndex.mat (if measured data is used).
...
% filePath='YBCO_LAO';             % name of the folder with the data
% 
% nMeasurements=3;              % number of measurements, hardscripted in my case
% 
...
addpath('D:\nigel\THz-TDS-Bruker_Vertex-Cuprates\Experiments\100nm_YBCO_on_LAO0p5mm-02-10-2020\Analysis\5484205\9485749_SubstrateReferencedTDS\SubstrateReferencedTDS\YBCO_LAO')
referenceTraces=importdata('reference_100K.txt');
sampleTraces=importdata('sample_100K.txt');
...
refractiveIndexFile=importdata('n_LAO.txt');
refractiveIndex=refractiveIndexFile(:,2);  % the refractive index of the substrate used
...
endDirectTransmission=277; % final step of the window.
...
lowerBound=0;     % [um] lower boundary for the search of the thickness difference, refine boundaries b
upperBound=50;      % [um] upper boundary
...
minimumFrequency=0.25;       % [THz] minimum frequency where Signal is stable, check by looking at the mismatch depending on frequency plot
maximumFrequency=2;       % [THz] maximum frequency where Signal is stable, 
...
figure('name','conductivites')
 ...
 result_complex_sheetconductivity_error=[realSheetConductivityTrimmedMean,imagSheetConductivityTrimmedMean(:,[2:1:3])];
 result_E_field_transmission=tI;
 
%  dlmwrite('sheet_cond_20K.txt', result_complex_sheetconductivity_error);
%  dlmwrite('E_field_transmission_20K.txt', result_E_field_transmission);
